...
class SnowEncode extends BitFilter {
	...
	public boolean flush() {
		...
		if (!quiet_flag) {
			...
			if (lines_extra > 0) {
				System.err.println("Message exceeded available space by approximately " + (usage - 100.0) + "%.");
				System.err.println("An extra  " + lines_extra + " lines were added.");
			} else {
				System.err.println("Message used approximately " + usage + "% of available space.");
			}
		}
	...
	public void space_calculate() {
		...
		if (storage[0] == storage[1]) {
			System.out.println("File has storage capacity of " + storage[0] + " bits (" + storage[0] / 8 + " bytes)");
		} else {
			System.out.println("File has storage capacity of between " + storage[0] + " and " + storage[1] + " bits.");
			System.out.println("Approximately " + (storage[0] + storage[1]) / 16 + " bytes.");
		}
	}
