...
class IceKey {
	...
	private static final int sMod[][] = { { 333, 313, 505, 369 }, { 379, 375, 319, 391 }, { 361, 445, 451, 397 }, { 397, 425, 395, 505 } };

	private static final int sXor[][] =
			{ { 0x83, 0x85, 0x9b, 0xcd }, { 0xcc, 0xa7, 0xad, 0x41 }, { 0x4b, 0x2e, 0xd4, 0x33 }, { 0xea, 0xcb, 0x2e, 0x04 } };

	private static final int pBox[] = { 0x00000001, 0x00000080, 0x00000400, 0x00002000, 0x00080000, 0x00200000, 0x01000000, 0x40000000,
			0x00000008, 0x00000020, 0x00000100, 0x00004000, 0x00010000, 0x00800000, 0x04000000, 0x20000000, 0x00000004, 0x00000010, 0x00000200,
			0x00008000, 0x00020000, 0x00400000, 0x08000000, 0x10000000, 0x00000002, 0x00000040, 0x00000800, 0x00001000, 0x00040000, 0x00100000,
	...
	private void scheduleBuild(int kb[], int n, int krot_idx) {
		...
		for (i = 0; i < 8; i++) {
			int j;
			int kr = IceKey.keyrot[krot_idx + i];
	...
	private void spBoxInit() {
		...
		for (i = 0; i < 1024; i++) {
			...
			x = gf_exp7(col ^ IceKey.sXor[0][row], IceKey.sMod[0][row]) << 24;
			IceKey.spBox[0][i] = perm32(x);

			x = gf_exp7(col ^ IceKey.sXor[1][row], IceKey.sMod[1][row]) << 16;
			IceKey.spBox[1][i] = perm32(x);

			x = gf_exp7(col ^ IceKey.sXor[2][row], IceKey.sMod[2][row]) << 8;
			IceKey.spBox[2][i] = perm32(x);

			x = gf_exp7(col ^ IceKey.sXor[3][row], IceKey.sMod[3][row]);
			IceKey.spBox[3][i] = perm32(x);
		}
	}
}
